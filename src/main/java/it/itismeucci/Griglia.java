...
public class Griglia {
    ...
    public char verificaVincitore() { // controlla tutta la griglia per capire chi è il vincitore
        for (int i = 0; i < grandezzaGriglia; i++) {
            if (grigliaPrincipale[i][0] == grigliaPrincipale[i][1] && grigliaPrincipale[i][1] == grigliaPrincipale[i][2]
                    && grigliaPrincipale[i][0] != VUOTO) {
                return grigliaPrincipale[i][0]; // returna X o O a seconda del carattere in quella posizione che abbiamo
                                                // controllato
            }
            if (grigliaPrincipale[0][i] == grigliaPrincipale[1][i] && grigliaPrincipale[1][i] == grigliaPrincipale[2][i]
                    && grigliaPrincipale[0][i] != VUOTO) {
                return grigliaPrincipale[0][i];
            }
        }
        // controlla le linee in diagonale
        if (grigliaPrincipale[0][0] == grigliaPrincipale[1][1] && grigliaPrincipale[1][1] == grigliaPrincipale[2][2]
                && grigliaPrincipale[0][0] != VUOTO) {
            return grigliaPrincipale[0][0];
        }
        if (grigliaPrincipale[0][2] == grigliaPrincipale[1][1] && grigliaPrincipale[1][1] == grigliaPrincipale[2][0]
                && grigliaPrincipale[0][2] != VUOTO) {
